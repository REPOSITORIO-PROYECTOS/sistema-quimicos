...
const handleDownloadFormulas = async () => {
    ...
    try {
        // Llamamos al nuevo endpoint en el blueprint de reportes
        ...
        // Procesamos la respuesta como un archivo blob
        const blob = await response.blob();
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        
        // Extraemos el nombre del archivo de las cabeceras si está disponible, si no, usamos uno por defecto
        ...
        setDownloadFormulasError(error.message);
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto flex-wrap justify-end">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2 flex-wrap">
                ...
                <button
                    ...
                    disabled={!token || allItems.length === 0 || isDownloadingFormulas}
                    className="w-full sm:w-auto bg-amber-600 text-white px-4 py-2 rounded-md shadow-sm hover:bg-amber-700 disabled:bg-gray-400 flex items-center justify-center gap-2"
                >
                    {isDownloadingFormulas ? '...' : 'Descargar Fórmulas'}
                </button>
                <button
