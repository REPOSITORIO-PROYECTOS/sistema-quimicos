...
// CAMBIO: Se define el vendedor fijo para esta página.
...
export default function RegistrarPedidoPage() {
  ...
  const resetearFormulario = useCallback(() => {
    const fechaEmisionEstandar = new Date().toISOString().slice(0, 16);
  ...
  // --- Las funciones de cálculo (useEffect y recalculatePricesForProducts) se mantienen sin cambios ---
useEffect(() => {
    const recalcularTodo = async () => {
      ...
      // LA ESTRUCTURA CORRECTA EMPIEZA AQUÍ
      try {
        // Todo el código que puede fallar (los fetch) va dentro del 'try'
        ...
        if (!resTotal.ok) {
            const errorData = await resTotal.json().catch(() => ({ error: "Error de red" }));
            throw new Error(errorData.error || `Error ${resTotal.status}`);
        }
        ...
        const montoFinalParaVueltoNeto = Math.max(0, montoConRecargosBruto * (1 - ((formData.descuentoTotal || 0) / 100)));
        ...
        if (formData.montoPagado >= montoFinalParaVueltoNeto && montoFinalParaVueltoNeto > 0) {
            const resVuelto = await fetch("https://quimex.sistemataup.online/ventas/calcular_vuelto", {
                method: "POST", headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
                body: JSON.stringify({ monto_pagado: formData.montoPagado, monto_total_final: montoFinalParaVueltoNeto }),
            });

            if (!resVuelto.ok) {
                const errorData = await resVuelto.json().catch(() => ({ error: "Error de red" }));
                throw new Error(errorData.error || `Error ${resVuelto.status}`);
            }
      ...
      } catch (error) { // El 'catch' viene DESPUÉS de cerrar el 'try'
        // Y aquí usamos la comprobación de tipo segura que discutimos
        if (error instanceof Error) {
            setErrorMessage(error.message);
        } else {
            setErrorMessage("Ocurrió un error desconocido al recalcular.");
        }
        setTotalCalculadoApi(null); // Si hay un error, reseteamos el total de la API

      } finally { // El 'finally' viene al final de todo
        // Este bloque SIEMPRE se ejecuta y su trabajo es quitar el "calculando..."
        setIsCalculatingTotal(false);
      }
    };
  ...
  const recalculatePricesForProducts = useCallback(async (currentProducts: ProductoPedido[]) => {
    ...
    currentProducts.forEach((p, index) => {
        if (p.producto > 0 && p.qx >= 0) {
            ...
            existing.totalQuantity += p.qx; existing.indices.push(index);
    ...
    const pricePromises = Array.from(productQuantities.entries()).map(
        async ([productoId, { totalQuantity, indices }]) => {
            try {
                if (totalQuantity === 0) return { precioUnitario: 0, precioTotalCalculado: 0, indices };
                ...
                return { precioUnitario: precioData.precio_venta_unitario_ars || 0, precioTotalCalculado: precioData.precio_total_calculado_ars || 0, indices };
          } catch (error) { 
              if (error instanceof Error) {
    ...
    priceResults.forEach(result => {
        ...
        indices.forEach(index => {
            ...
            const baseTotal = precioTotalCalculado > 0 ? precioTotalCalculado : (precioUnitario * item.qx);
            item.total = baseTotal * (1 - (item.descuento / 100));
        });
    });
    setProductos(updatedProducts);
  }, [setErrorMessage]);
  
  // --- Los handlers de producto y formulario se mantienen sin cambios ---
  ...
  const eliminarProducto = useCallback(async (index: number) => {
    ...
    if (nuevosProductos.length === 0) {
        nuevosProductos.push({ ...initialProductos[0] });
    }
    await recalculatePricesForProducts(nuevosProductos);
  }, [productos, recalculatePricesForProducts]);
  ...
  const handleSubmit = async (e: React.FormEvent ) => {
    ...
    if (!formData.clienteId) { setErrorMessage("Seleccione un cliente."); setIsSubmitting(false); return; }
    if (productos.every(p => p.producto === 0 || p.qx === 0)) { setErrorMessage("Añada al menos un producto."); setIsSubmitting(false); return; }
    if (!totalCalculadoApi && montoBaseProductos > 0) { setErrorMessage("Error calculando el total final."); setIsSubmitting(false); return; }
    ...
    const montoTotalFinalNeto = Math.max(0, montoConRecargosBruto * (1 - ((formData.descuentoTotal || 0) / 100)));
    ...
    const dataPayload = {
      ...
      cliente_id: clienteIdParaApi, fecha_pedido: formData.fechaEmision, direccion_entrega: formData.direccion,
      cuit_cliente: formData.cuit,
      nombre_vendedor: VENDEDOR_FIJO, // <-- CAMBIO: Se usa el vendedor fijo
      monto_pagado_cliente: formData.montoPagado, forma_pago: formData.formaPago, vuelto: formData.vuelto, 
      requiere_factura: formData.requiereFactura, monto_total_base: montoBaseProductos, 
      descuento_total_global_porcentaje: formData.descuentoTotal,
      monto_final_con_recargos: parseFloat(montoTotalFinalNeto.toFixed(2)), observaciones: formData.observaciones || "",
    };
      ...
      if (response.ok) {
        ...
        setErrorMessage(result.message || result.detail || result.error || `Error ${response.status}`);
          ...
          if (err instanceof Error) {
              ...
              setErrorMessage("Error de red o desconocido al registrar.");
          }
      } finally {
  ...
  const handleImprimirPresupuesto = (pedidoIdParaImprimir: number) => {
    document.title = `Pedido QuiMex - N° ${pedidoIdParaImprimir}`;
    window.print();
  };

  if (loadingClientes) { return <div className="flex items-center justify-center min-h-screen bg-indigo-900"><p className="text-white text-xl">Cargando...</p></div>; }
  if (errorClientes) { return <div className="flex flex-col items-center justify-center min-h-screen bg-red-900 text-white p-4"> <h2 className="text-2xl font-bold mb-4">Error al Cargar Clientes</h2> <p className="bg-red-700 p-2 rounded mb-4 text-sm">{errorClientes}</p> <button onClick={() => window.location.reload()} className="bg-white text-red-900 px-4 py-2 rounded">Reintentar</button> </div>; }
  ...
  const ventaDataParaTicket: VentaData = {
      ...
      vendedor: VENDEDOR_FIJO, // <-- CAMBIO: Se usa el vendedor fijo para el ticket
      items: productos
          .filter(p => p.producto && p.qx > 0)
          .map(item => {
              const pInfo = productosContext?.productos.find(p => p.id === item.producto);
              const totalConRecargosItem = item.total * (totalCalculadoApi && montoBaseProductos > 0 ? totalCalculadoApi.monto_final_con_recargos / montoBaseProductos : 1);
              return {
        ...
        <div className="bg-white p-6 md:p-8 rounded-lg shadow-md w-full max-w-5xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <fieldset className="border p-4 rounded-md">
              ...
              {/* CAMBIO: Se elimina el selector de vendedor y se ajusta el grid */}
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <input type="datetime-local" name="fechaEmision" id="fechaEmision" disabled value={formData.fechaEmision}
            ...
            <fieldset className="border p-4 rounded-md">
              ...
              <div className="mb-2 hidden md:grid md:grid-cols-[minmax(0,0.7fr)_80px_70px_minmax(0,0.5fr)_100px_100px_32px] items-center gap-x-2 font-semibold text-sm text-gray-600 px-1 md:px-3">
                <span>Producto*</span><span className="text-center">Cant*</span><span className="text-center">Desc%</span><span>Observ. Prod.</span>
                <span className="text-right">P.Unit</span><span className="text-right">Total</span><span />
              </div>
              <div className="space-y-3">
                {productos.map((item, index) => (
                  <div key={index} className="grid grid-cols-1 md:grid-cols-[minmax(0,0.7fr)_80px_70px_minmax(0,0.5fr)_100px_100px_32px] items-center gap-x-2 gap-y-1 border-b pb-2 last:border-b-0 md:border-none md:pb-0">
                    ...
                    <input type="number" name="descuento" placeholder="0" value={item.descuento === 0 ? '' : item.descuento} onChange={(e) => handleProductRowInputChange(index, e)}
                      min="0" max="100" className="shadow-sm border rounded w-full py-2 px-2 text-gray-700 text-sm text-center focus:outline-none focus:ring-1 focus:ring-indigo-500 no-spinners" />
                    ...
                    <input type="text" value={`$ ${item.precio.toFixed(2)}`} readOnly className="shadow-sm border rounded w-full py-2 px-2 text-gray-700 text-sm text-right bg-gray-100"/>
                    <input type="text" value={`$ ${item.total.toFixed(2)}`} readOnly className="shadow-sm border rounded w-full py-2 px-2 text-gray-700 text-sm text-right bg-gray-100"/>
             ...
             <fieldset className="border p-4 rounded-md">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                ...
                <div>
                  ...
                  <input id="descuentoTotal" type="number" name="descuentoTotal" className="w-full bg-white shadow-sm border rounded py-2 px-3 text-gray-700 focus:outline-none focus:ring-1 focus:ring-indigo-500 no-spinners"
                      value={formData.descuentoTotal === 0 ? '' : formData.descuentoTotal} onChange={handleFormChange} placeholder="0" step="1" min="0" max="100"/>
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="montoPagado">Monto Pagado</label>
                    <input id="montoPagado" type="number" name="montoPagado" className="w-full bg-white shadow-sm border rounded py-2 px-3 text-gray-700 focus:outline-none focus:ring-1 focus:ring-indigo-500 no-spinners"
                        value={formData.montoPagado === 0 ? '' : formData.montoPagado} onChange={handleFormChange} placeholder="0.00" step="0.01" min="0"/>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="vuelto">Vuelto</label>
                  <input id="vuelto" type="text" name="vuelto" readOnly className="w-full bg-gray-100 shadow-sm border rounded py-2 px-3 text-gray-700 focus:outline-none text-right" value={`$ ${formData.vuelto.toFixed(2)}`}/>
                </div>
              </div>
